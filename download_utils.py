...
def _set_permission(path: pathlib.Path, target_permission: int):
    ...
    if path.stat().st_mode & target_permission == target_permission:
        rank0_print(f"Skipping {path} because it already has correct permissions")
        return
    path.chmod(target_permission)
    rank0_print(f"Set {path} to {target_permission}")
...
def get_cache_dir() -> pathlib.Path:
    default_dir = "/iris/u/ajaysri/.cache/openpi"
    if os.path.exists("/mnt/weka"):  # noqa: PTH110
        default_dir = f"/mnt/weka/{getpass.getuser()}/.cache/openpi"

    cache_dir = pathlib.Path(default_dir).expanduser().resolve()
...
def maybe_download(url: str, *, force_download: bool = False, **kwargs) -> pathlib.Path:
    ...
    try:
        ...
        with filelock.FileLock(lock_path):
            ...
            if invalidate_cache:
                rank0_print(f"Removing expired cached entry: {local_path}")
            ...
            rank0_print(f"Downloading {url} to {local_path}")
