...
def convert_content_list_to_text(content_list: List[Dict[str, Any]]) -> tuple[str, List[str]]:
    """
        ...
        if "text" in content_item:
            ...
            if isinstance(frames, list):
                for frame in frames:
                    ...
                    media_files.append(frame)
            else:
                text_parts.append("<image>")
                media_files.append(frames)
            ...
            if isinstance(images, list):
                ...
                text_parts.append("<image>")
                media_files.append(images)
    
    combined_text = "\n".join(text_parts).strip()
    return combined_text, media_files


def convert_vla_item(vla_item: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    for message in message_list:
        ...
        if not isinstance(content, list):
            ...
            content_text, media_files = convert_content_list_to_text(content)
        
        # Map role
        mapped_role = role_mapping.get(role, role)
        
        # Skip system messages for now (they're handled separately in preprocessing)
...
def convert_vla_dataset(input_file: str, output_file: str):
    """
    ...
    for i, vla_item in enumerate(vla_data):
        try:
            converted_item = convert_vla_item(vla_item)
...
def main():
    ...
    if args.sample_size:
        ...
        try:
            convert_vla_dataset(sample_file, args.output_file)
        finally:
            # Clean up sample file
            if os.path.exists(sample_file):
                os.remove(sample_file)
    else:
        convert_vla_dataset(args.input_file, args.output_file)
